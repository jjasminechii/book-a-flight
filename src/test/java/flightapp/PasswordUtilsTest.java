...
public class PasswordUtilsTest extends TestCase {
  ...
  @Test
  public void testCanMatchCompPlaintextToHash() {
    byte[] hash = PasswordUtils.hashPassword("plant");
    byte[] extractedSalt = Arrays.copyOfRange(hash, 0, 16);
    byte[] hashed = Arrays.copyOfRange(hash, 16, hash.length);
    byte[] hashedPlainText = PasswordUtils.generateSaltedPassword("plant", extractedSalt);
    System.out.println(hash.toString());
    System.out.println(hashedPlainText.toString());
    System.out.println(hashed.toString());
    assertTrue(Arrays.equals(hashed, hashedPlainText));
  }
